...
void test_cpp_func(const int v) {
  printf("Hello, World!\nYour number is %d!\n", v);
}
...
extern "C" {
void ReadAddressBook(const char* filename, const char** ) {
  std::string str;
  tutorial::AddressBook address_book;
  ReadAddressBook(&address_book, std::string(filename));
  address_book.SerializeToString(&str);
  return str.c_str();
}
}
